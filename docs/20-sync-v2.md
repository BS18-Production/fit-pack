...
| **7 — Durum ve operasyon** ✅ | `classifySyncError` (ulaşılamıyor / oturum yok / kalıcı / onarılabilir / geçici — tanınmayan hata **geçici** sayılır); kalıcı hatada toplu gönderim satır satır ayrıştırılır, bozuk satır `sync_state = 2` olur ve **kuyruğun geri kalanı durmaz**; `SyncHealth` son başarılı gönderim/çekmeyi ve kesinti başlangıcını defterde tutar (kayıt `pushAll`/`pullAll` sonunda, kim çağırırsa çağırsın); hesap ekranında "Son yedekleme: bugün 14:32", 3 günden uzun kesintide uyarı, hatalı satırda dokununca "tekrar dene". **2026-09-19**, 26 test (toplam 436). Simülatörde görsel doğrulama **bekliyor** (Mac ekranına erişilemedi). | C-36, duraklatma | yok | ✅ |
