...
class BackupService {
  ...
  Future<void> shareBackup() async {
    ...
    await Share.shareXFiles(
      ...
      subject: 'Fit Pack yedek',
      text: 'Fit Pack veri yedeği — geri yüklemek için saklayın.',
    );
  }
  ...
  Future<void> restoreFromFile(File picked) async {
    if (!await _isSqlite(picked)) {
      throw const FormatException('Geçerli bir Fit Pack yedeği değil.');
    }
    ...
    if (backupVersion > db.schemaVersion) {
      throw const FormatException(
          'Bu yedek daha yeni bir Fit Pack sürümüyle alınmış. '
          'Önce uygulamayı güncelle.');
    }
